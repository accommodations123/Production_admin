...
const HostingApproval = () => {
  ...
  const normalize = (item) => {
    ...
    return {
      ...
      listing_expires_at: raw?.listing_expires_at ?? null,
        ...
        <div className="relative h-52 w-full bg-gradient-to-br from-slate-100 to-slate-200">
          ...
          <div className="absolute top-3 right-3 flex flex-col gap-2 items-end">
            <span className="inline-flex items-center rounded-lg bg-white/90 backdrop-blur-sm px-2.5 py-1 text-xs font-bold uppercase tracking-wider text-slate-700 shadow-sm ring-1 ring-slate-900/5">
              {property.status === "approved" && (property.is_expired || (property.listing_expires_at && new Date(property.listing_expires_at).getTime() < Date.now()))
                ? "expired"
                : property.status}
            </span>
          </div>
        ...
        <div className="flex flex-1 flex-col p-5">
          ...
          <div className="mt-5 flex items-center justify-between gap-2 pt-4 border-t border-slate-100">
            ...
            {property.status === "approved" && (() => {
              const isExpired = property.is_expired || (property.listing_expires_at && new Date(property.listing_expires_at).getTime() < Date.now());
              if (isExpired) {
                return (
                  <span className="flex items-center gap-1.5 text-xs font-bold text-rose-600 bg-rose-50 px-2.5 py-1 rounded-lg">
                    <ExclamationCircleIcon className="h-3.5 w-3.5" />
                    Expired ({property.listing_expires_at ? new Date(property.listing_expires_at).toLocaleDateString() : 'Unknown'})
                  </span>
                );
              }
              if (property.listing_expires_at) {
                const diffTime = new Date(property.listing_expires_at).getTime() - Date.now();
                ...
                return (
                  <span className="flex items-center gap-1.5 text-xs font-bold text-amber-600 bg-amber-50 px-2.5 py-1 rounded-lg">
                    ...
                    {diffDays > 0 ? `${diffDays} days left` : 'Expiring today'}
                  </span>
                );
              }
              return (
        ...
        <div className="flex flex-1 flex-col justify-between p-6">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
            <div className="flex-1 min-w-0">
              ...
              {property.status === "approved" && property.listing_expires_at && (
                <div className="mt-3 flex items-center gap-2 text-sm">
                  {(() => {
                    const isExpired = property.is_expired || new Date(property.listing_expires_at).getTime() < Date.now();
                    if (isExpired) {
                      return (
                        <span className="flex items-center gap-1.5 text-xs font-bold text-rose-600 bg-rose-50 px-2.5 py-1 rounded-lg border border-rose-200/50">
                          <ExclamationCircleIcon className="h-3.5 w-3.5" />
                          Expired on {new Date(property.listing_expires_at).toLocaleDateString()}
                        </span>
                      );
                    }
                    const diffTime = new Date(property.listing_expires_at).getTime() - Date.now();
                    const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
                    return (
                      <span className="flex items-center gap-1.5 text-xs font-bold text-amber-600 bg-amber-50 px-2.5 py-1 rounded-lg border border-amber-200/50">
                        <ClockIcon className="h-3.5 w-3.5" />
                        {diffDays > 0 ? `${diffDays} days left` : 'Expiring today'}
                      </span>
                    );
