        ...
        <div className="relative h-60 w-full overflow-hidden">
          ...
          <div className="absolute top-4 left-4 flex flex-wrap gap-2">
            ...
            {p.listing_expires_at && (() => {
              const isExpired = p.is_expired || new Date(p.listing_expires_at).getTime() < Date.now();
              if (isExpired) {
                return (
                  <span className="px-3 py-1 bg-rose-500/80 backdrop-blur-md text-white text-xs font-bold rounded-lg border border-rose-500/30 uppercase tracking-wider">
                    Expired
                  </span>
                );
              }
              const diffTime = new Date(p.listing_expires_at).getTime() - Date.now();
              const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
              return (
                <span className="px-3 py-1 bg-amber-500/80 backdrop-blur-md text-white text-xs font-bold rounded-lg border border-amber-500/30 uppercase tracking-wider">
                  {diffDays > 0 ? `${diffDays}d left` : 'Expiring today'}
                </span>
              );
            })()}
          </div>
                  ...
                  >
                    ...
                    <div className="flex-1 p-6 flex flex-col justify-between">
                      <div>
                        ...
                        {item.property.listing_expires_at && (
                          <div className="text-xs font-semibold mb-3">
                            {(() => {
                              const isExpired = item.property.is_expired || new Date(item.property.listing_expires_at).getTime() < Date.now();
                              if (isExpired) {
                                return <span className="text-rose-400">Expired on {new Date(item.property.listing_expires_at).toLocaleDateString()}</span>;
                              }
                              const diffTime = new Date(item.property.listing_expires_at).getTime() - Date.now();
                              const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
                              return <span className="text-amber-400">Active: {diffDays > 0 ? `${diffDays} days left` : 'Expiring today'}</span>;
                            })()}
                          </div>
                        )}
