...
// --- CONFIG ---
const API_URL = "https://api.nextkinlife.live/admin/approved/approved-host-details";
const BRAND_COLORS = {
      ...
      <div className="max-w-[1600px] mx-auto p-4 sm:p-8">
        ...
        <header className="bg-white/80 backdrop-blur-xl border border-slate-200 sticky top-0 z-40 rounded-2xl shadow-sm px-6 py-4 mb-8 flex justify-between items-center transition-all">
          ...
          <div className="flex items-center gap-3">
            ...
            <button className="flex items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-slate-900 to-slate-800 text-white rounded-xl shadow-lg hover:shadow-xl hover:-translate-y-0.5 transition-all font-medium">
              <Plus size={18} />
              <span>Add New</span>
            </button>
          </div>
          ...
          {notificationPanelOpen && (
            ...
            >
              <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                ...
                <button onClick={markAllNotificationsAsRead} className="text-xs font-semibold text-slate-900 hover:underline">Mark all read</button>
              </div>
            ...
            <div className="flex flex-col md:flex-row justify-between items-center bg-white p-4 rounded-2xl shadow-sm border border-slate-100 mb-8 gap-4">
              <div className="relative w-full md:w-1/3 group">
                ...
                <input
                  ...
                  className="w-full pl-12 pr-4 py-3 bg-slate-50 border-none rounded-xl focus:ring-2 focus:ring-slate-900 transition-all font-medium placeholder:text-slate-400"
                  value={searchTerm}
              ...
              <div className="flex items-center gap-2 w-full md:w-auto">
                <button
                  ...
                  className={`flex-1 md:flex-none flex items-center justify-center gap-2 px-4 py-3 rounded-xl font-medium transition-all ${filterPanelOpen ? 'bg-slate-900 text-white shadow-lg' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
                >
                  <Filter size={18} />
                  <span>Filters</span>
                </button>
                <div className="flex bg-slate-100 p-1 rounded-xl">
                  {[
                    ...
                    <button
                      ...
                      className={`p-2.5 rounded-lg transition-all ${viewMode === mode ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                    >
                      <Icon size={18} />
                    </button>
                  ))}
              ...
              {viewMode === VIEW_MODES.GRID ? (
                ...
                >
                  {filteredCategories.map((category, index) => (
                    ...
                    >
                      ...
                      <div className="h-24 bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-6 relative overflow-hidden">
                  ...
                  {filteredCategories.map((category, index) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="p-2 rounded-full bg-slate-50 group-hover:bg-slate-900 group-hover:text-white transition-all">
                ...
                >
                  ...
                  <div className="z-10 text-center">
                    <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-slate-900 to-slate-800 flex items-center justify-center mx-auto mb-6 shadow-lg text-white">
                      <Map size={40} />
                    </div>
              ...
              <form onSubmit={handlePropertyFormSubmit} className="p-8 grid grid-cols-1 md:grid-cols-2 gap-6">
                {[
                  ...
                  <div key={field.name} className="space-y-2">
                    ...
                    {field.type === 'select' ? (
                      <select name={field.name} value={formData[field.name]} onChange={handleInputChange} className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-slate-900 transition-all font-medium">
                        <option value="">Select Type</option>
                        {categories.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                      </select>
                    ) : (
                      <input type={field.type} name={field.name} value={formData[field.name]} onChange={handleInputChange} className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-slate-900 transition-all font-medium placeholder:text-slate-400" />
                    )}
                  </div>
                ))}
                ...
                <div className="md:col-span-2 space-y-2">
                  ...
                  <textarea name="propertyDescription" value={formData.propertyDescription} onChange={handleInputChange} rows={4} className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-slate-900 transition-all font-medium"></textarea>
                </div>
                ...
                <div className="md:col-span-2 flex justify-between items-center pt-4 border-t border-slate-100">
                  ...
                  <button type="submit" className="px-8 py-3 bg-slate-900 text-white font-bold rounded-xl hover:bg-slate-800 hover:shadow-lg transition-all">Save Property</button>
                </div>
              </form>
            </motion.div>
          </motion.div>
